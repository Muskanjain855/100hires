...
- 4 YouTube videos verified as real (title + channel confirmed via YouTube's public oEmbed API); transcripts not yet pulled — a script (`research/other/collect_youtube_transcripts.py`) is included to fetch them via `youtube-transcript-api` once run somewhere with normal internet access.
- Known gaps are tracked, not hidden: see `research/other/collection-notes.md` for the full punch list (missing links for 4 items, 2 "fit flags," 2 "gap flags," and 1 unverified stat).
...
## Status
This is the source-collection phase, not the final playbook. Next: fill in the "NOT YET COLLECTED" placeholders, run the transcript script, and resolve the flagged gaps before drawing conclusions.
